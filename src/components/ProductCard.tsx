...
const ProductCard: React.FC<ProductCardProps> = ({ product, onOrderClick }) => {
  ...
  const { variant } = product; // Destructure the variant directly
      ...
      <div className="p-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-gray-700">{variant.unitSize}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {isAdmin ? (
              ...
              <button
                className={`inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md transition-colors duration-200 ${variant.stockQuantity > 0
              ...
              >
                {variant.stockQuantity > 0 ? 'Order' : 'Out of Stock'}
              </button>
            )}
          </div>
        </div>
